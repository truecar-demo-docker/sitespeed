  ...
  return new Promise((resolve, reject) => {
    ...
    if (groups) {
      if (dynamodbTableName) {
        ...
        // Hash the object for a unique key
        const objectJSON = JSON.stringify(dynamoEntry);
        const hash = crypto.createHash('sha1').update(objectJSON).digest('hex');
        dynamoEntry.Item.primaryKey = {'S': hash.substring(0,8)};
