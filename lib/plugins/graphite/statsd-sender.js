...
class StatsDSender extends Sender {
  ...
  bulk(data) {
    ...
    */
    ...
    const pageTimingsR = new RegExp(/^statistics[.](cpu|timings)[.](categories|events|longTasks|pageTimings|paintTiming)[.](.*)/);
    const pageSummaryR = new RegExp(/(.*[.]pageSummary)[.]([^.]+)[.]([^.]+)[.]([^.]+)[.]([^.]+)[.]([^.]+)[.]?(.*?)[.]?(\d+|avg|max|mdev|mean|median|min|p\d+|stddev|total)?[:]([-]?\d+)/);
      ...
      data.split('\n').forEach(function(line) {
        if (line) {
          ...
          if (extractParts) {
            ...
            if (!("aggType" in matchObj) || [undefined, 'median', 'max', 'min'].includes(matchObj.aggType)) {
