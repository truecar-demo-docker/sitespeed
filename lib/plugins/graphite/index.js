...
module.exports = {
  ...
  open(context, options) {
    ...
    defaultConfig.port = isStatsd(options.graphite) ? 8125 : 2003;
    ...
    const senderName = SenderConstructor.facility;
    ...
    log.debug(
      'Setting up %s %s:%s for namespace %s',
      senderName,
      opts.host,
      opts.port,
      opts.namespace
  ...
  processMessage(message, queue) {
    ...
    // we only sends individual groups to senderName, not the
